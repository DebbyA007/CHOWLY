...
import { MARK_ARC_START, MARK_DOT, MARK_OCHRE, MARK_PATH, MARK_STROKE, MARK_VIEWBOX } from "./brand.ts";
...
test("the mark in the app is the mark in public/brand/mark.svg", () => {
  ...
  assert.ok(MARK_PATH.startsWith(`M ${MARK_ARC_START.x} ${MARK_ARC_START.y} `), "arc start");
